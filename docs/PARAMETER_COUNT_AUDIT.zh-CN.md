...
这些方法没有一个可用于“参数量应完全相同”的外部原仓库模型。它们以 Hugging Face `UNet2DConditionModel` 为基础，由本项目设置通道、层数、DCC、WWFCA或物理展开模块。只能核对实现是否符合本项目配置：

| 本地方法/配置 | 参数量 | 与外部原仓库比较 |
|---|---:|---|
| HF small baseline | 1,395,457 | HF 类的本地小规格实例，无官方固定模型 |
| Chen-HF full | 1,396,413 | 本项目新增改进，无同构上游模型 |
| Chen-HF no-sparse | 1,395,818 | 本项目消融，无同构上游模型 |
| HF matched（四层 128） | 13,906,945 | HF 类的本地匹配规格实例 |
| DCC-only | 13,908,097 | 本项目消融；多 1,152 来自额外条件输入通道 |
| WWFCA-only | 13,906,945 | 参数量与 HF matched 相同，但计算图不同 |
| FDU（DCC+WWFCA） | 13,908,097 | 本项目方法，无外部同构原版 |
