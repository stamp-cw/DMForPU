...
class Evaluator:
    ...
    def _save_eval_pt(self):
        ...
        pt_path = self.config.io.generated_eval_pt_file_path(self.saved_samples, self.saved_samples + self.temp_batch_size)
        torch.save(self.samples, pt_path)
        self.logger.info(f"Saved {self.temp_batch_size} samples to {pt_path}")

    def _save_eval_png(self):
        samples = self.samples.permute(0, 3, 1, 2)
        for _, img_array in enumerate(samples):
            img = ToPILImage()(img_array)
            img_path = self.config.io.generated_eval_png_file_path(self.saved_samples + 1)
            img.save(img_path)
        self.logger.info(f"Saved {self.samples.shape[0]} samples as PNG images in folder: {self.config.io.out_raw_eval_path}")


    def _save_eval_and_preview(self, raw_images, mask_images):
        self.samples = mask_images
        self.samples = torch.clamp(self.samples.permute(0, 2, 3, 1).cpu() * 255, 0, 255).to(torch.uint8)
        samples_to_visualize = self.samples[:min(36, len(self.samples))]
        visualize_samples_file_path = self.config.io.generated_eval_pdf_file_path(self.saved_samples, self.saved_samples + self.temp_batch_size, 0)
        ...
        visualize_raw_file_path = self.config.io.generated_eval_pdf_file_path(self.saved_samples, self.saved_samples + self.temp_batch_size, 1)
        ...
        assert raw_images.shape[0] == mask_images.shape[0]
        # visualize_merged_file_path = self.config.io.sample_pdf_file_path(3)
        visualize_merged_file_path = self.config.io.generated_eval_pdf_file_path(self.saved_samples, self.saved_samples + self.temp_batch_size, 3)
        images_list = []
        for i in range(raw_images.shape[0]):
            raw = ToPILImage()(raw_images[i].cpu())
            mask = ToPILImage()(mask_images[i].cpu())
            # 横向拼接
            width, height = raw.width + mask.width, raw.height
            merged = Image.new('RGB', (width, height))
            merged.paste(raw, (0, 0))
            merged.paste(mask, (raw.width, 0))
    ...
    def saved_samples(self):
        return self.config.io.latest_generated_eval_num
    ...
    def total_samples(self):
        return self.eval_loader.dataset.__len__()