"""Run WWFCA-v4 and its exact disabled control sequentially."""
...
def main():
    ...
    for index, method in enumerate(METHODS):
        ...
        while process.poll() is None:
            write_status({"state": "training", "active": method, "active_pid": process.pid,
                          ...
                          "elapsed_seconds": time.time() - started, "epochs_each": 25})
            time.sleep(20)
        stdout.close(); stderr.close()
        results[method] = {"exit_code": process.returncode,
    ...
    analysis = subprocess.call([sys.executable, "-B", str(ROOT / "experiments/analyze_gfs128_wwfca_v4.py")], cwd=ROOT)
