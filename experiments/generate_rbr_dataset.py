"""Generate the real-terrain based RBR v1 phase-unwrapping dataset."""
...
def render_scene(region: dict, source: np.ndarray, size: int, snr: float, rng: np.random.Generator, sim: dict) -> tuple[dict, dict]:
...
def write_file(path: Path, size: int, split: str, count: int, fixed_snr: int | None, seed: int, regions: list[dict], dataset_cfg: dict, sim: dict, maps: dict) -> dict:
    path.parent.mkdir(parents=True, exist_ok=True); temporary = path.with_suffix(".h5.tmp")
    if temporary.exists(): temporary.unlink()
    snrs = snr_vector(count, fixed_snr, seed); chunk = (min(16, count), size, size)
    ...
    with h5py.File(temporary, "w") as f:
        ...
        f.attrs.update(family="RBR", source_name="Copernicus GLO-30 real-terrain simulation", split=split, image_size=size, seed=seed, generator_version="1.0", noise_model=sim["noise"]["mode"], metadata_maps=json.dumps(maps), region_maps=json.dumps({r["config"]["name"]:r["id"] for r in regions}), satellite_maps=json.dumps({s["name"]:i for i,s in enumerate(sim["satellites"])}))
        for index in range(count):
            ...
            arrays, meta = render_scene(region, source, size, float(snrs[index]), rng, sim)
            ...
            if (index+1)%250==0 or index+1==count: print(f"RBR{size} {path.name}: {index+1}/{count}", flush=True)
    temporary.replace(path)
...
def validate_dataset(output: Path, sizes: list[int], train_count: int, test_count: int, seed: int) -> dict:
    ...
    files = [output/f"RBR{size}"/name for size in sizes for name in ["train.h5",*[f"test_{s}dB.h5" for s in load_yaml("dataset.yaml")["test_snrs_db"]]]]
    ...
    cross_snr=[]
    for size in sizes:
        with h5py.File(output/f"RBR{size}"/"test_0dB.h5","r") as a,h5py.File(output/f"RBR{size}"/"test_30dB.h5","r") as b:
            ids=rng.choice(test_count,min(20,test_count),replace=False);cross_snr.append(float(np.max(np.abs(a["phi"][sorted(ids)]-b["phi"][sorted(ids)]))))
    report={"state":"pass" if not failures and duplicates==0 and max(cross_snr)<1e-7 else "fail","random_samples_checked":len(chosen),"failures":failures,"unexpected_duplicate_clean_scenes":duplicates,"expected_cross_snr_scene_matches_ignored":True,"max_wrap_consistency_error":max(checks),"max_cross_snr_gt_difference":max(cross_snr)}
    atomic_json(output/"RBR_QA_REPORT.json",report)
    if report["state"]!="pass":raise RuntimeError(f"RBR QA failed: {report}")
    return report


def visualize(output: Path) -> Path:
    path=output/"RBR128"/"test_10dB.h5"
    ...
    preview=output/"RBR_preview.png";fig.savefig(preview,dpi=170);plt.close(fig);return preview
...
def update_global_manifest(output: Path, entries: dict, dataset_cfg: dict, regions_cfg: dict, qa: dict, elapsed: float) -> None:
    ...
    manifest["families"]["RBR"]={"status":"generated","generated":True,"version":"1.0","source":"Copernicus GLO-30 real terrain","sizes":entries,"protocol":{"train_count":dataset_cfg["train_count"],"validation_count_within_train":dataset_cfg["validation_count_within_train"],"test_count_per_snr":dataset_cfg["test_count_per_snr"],"train_snrs":dataset_cfg["train_snrs_db"],"test_snrs":dataset_cfg["test_snrs_db"],"spatial_split":True,"test_cross_region_fraction":dataset_cfg["test_cross_region_fraction"],"noise":"complex-domain, spatial coherence modulated"},"qa":qa,"elapsed_seconds":elapsed,"license_notice":regions_cfg["license_notice"]}
...
def generate(output: Path, train_count: int, test_count: int, sizes: list[int], seed: int, update_manifest: bool) -> None:
    ...
    for size in sizes:
        folder=output/f"RBR{size}";folder.mkdir(parents=True,exist_ok=True);files={}
    ...
    summary={"schema_version":1,"generator":"experiments/generate_rbr_dataset.py","seed":seed,"config_files":["configs/rbr/dataset.yaml","configs/rbr/regions.yaml","configs/rbr/simulation.yaml"],"sizes":entries,"qa":qa,"preview":None if preview is None else str(preview.relative_to(ROOT)) if preview.is_relative_to(ROOT) else str(preview),"elapsed_seconds":time.time()-started}
    atomic_json(output/"RBR_DATASET.json",summary)
    if update_manifest:update_global_manifest(output,entries,dataset_cfg,regions_cfg,qa,time.time()-started)
    print(f"RBR complete in {(time.time()-started)/60:.1f} min: {output}",flush=True)
