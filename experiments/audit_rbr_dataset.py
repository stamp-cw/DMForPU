"""Full-file QA and statistics for an already generated RBR dataset."""
...
def invariant_checks():
    ...
    for size in SIZES:
        base=DATA/f"RBR{size}"/"test_0dB.h5";maximum=0.;metadata=True
        with h5py.File(base,"r") as a:
            for snr in SNRS[1:]:
                with h5py.File(DATA/f"RBR{size}"/f"test_{snr}dB.h5","r") as b:
        ...
        checks["cross_snr"][str(size)]={"max_gt_difference":maximum,"metadata_identical":bool(metadata)}
    with h5py.File(DATA/"RBR128"/"train.h5","r") as base:
        for size in (64,32):
            with h5py.File(DATA/f"RBR{size}"/"train.h5","r") as other:
...
def plot(distributions,category_result):
    ...
    fig.suptitle("RBR128 training-set statistics");path=DATA/"RBR_statistics.png";fig.savefig(path,dpi=180);plt.close(fig);return path
...
def main():
    ...
    for size in SIZES:
        files[str(size)]={}
        for name,count in [("train",5000),*((f"test_{s}dB",1000) for s in SNRS)]:
            result,dist=audit_file(DATA/f"RBR{size}"/f"{name}.h5",count,collect=size==128 and name=="train");files[str(size)][name]=result
            if size==128 and name=="train":distributions=dist;category=result
            print(size,name,result["valid_ranges"],flush=True)
    invariants=invariant_checks();all_valid=all(x["valid_ranges"] for group in files.values() for x in group.values()) and all(x["max_gt_difference"]==0 and x["metadata_identical"] for x in invariants["cross_snr"].values()) and all(all(x.values()) for x in invariants["cross_size_train_metadata"].values())
    ...
    dump(DATA/"RBR_STATISTICS.json",stats)
    for manifest_path in (DATA/"RBR_DATASET.json",DATA/"DATASETS_V2.json"):
        obj=json.loads(manifest_path.read_text(encoding="utf-8"))
        target=obj if manifest_path.name=="RBR_DATASET.json" else obj["families"]["RBR"]
        target["full_audit"]={"state":stats["state"],"file":"data/RBR_STATISTICS.json","figure":"data/RBR_statistics.png"};dump(manifest_path,obj)
    if not all_valid:raise RuntimeError("RBR full audit failed")
