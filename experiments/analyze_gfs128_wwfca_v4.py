"""Build paired WWFCA-v4/off metrics and diagnostic plots."""
...
OUT = ROOT / "experiments/results/gfs128_wwfca_v4"
METHODS = ("wwfca_v4_off", "wwfca_v4")
...
def main():
    ...
    for method, history in histories.items():
        ...
        for metric in metrics:
            ...
            extrema.append({"method": method, "metric": metric,
                            "direction": "max" if metric in HIGHER else "min",
                            "epoch": row["epoch"], "value": row["val_" + metric]})
    write_csv(OUT / "paired_primary_metrics.csv", selected)
    write_csv(OUT / "paired_per_metric_best.csv", extrema)
    (OUT / "paired_summary.json").write_text(json.dumps(
        {"common_epochs": common, "selected": selected, "per_metric_best": extrema}, indent=2), encoding="utf-8")

    fig, axes = plt.subplots(2, 3, figsize=(18, 10), constrained_layout=True)
    panels = (("val_u3_aligned_nrmse", "U3-aligned NRMSE"),
              ("val_mean_aligned_nrmse", "Mean-aligned NRMSE"),
              ("val_u3_aligned_ssim", "U3-aligned SSIM"),
              ("gamma", "Residual gate gamma"),
              ("residual_rms_ratio", "Injected/base RMS"),
              ("attention_entropy", "Normalized attention entropy"))
    colors = {"wwfca_v4_off": "#377eb8", "wwfca_v4": "#2b8c6b"}
    ...
    axes[0, 0].legend(); fig.suptitle("GFS128 WWFCA-v4 controlled ablation")
    ...
    for method in METHODS:
        ...
        for condition, values in complete["tests"].items():
            tests.append({"method": method, "condition": condition,
                          "selected_epoch": complete["selected_epoch"], **values})
