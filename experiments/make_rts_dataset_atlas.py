"""Create the RTS dataset atlas from the released H5 files and GIS vectors."""
...
OUT = ROOT / "output" / "pdf"
PAGES = OUT / "rts_dataset_atlas_pages"
PDF = OUT / "rts_dataset_atlas.pdf"
META = OUT / "rts_dataset_atlas.json"
REGION_DIR = ROOT / "data" / "rts_sources" / "regions"
SNRS = (30, 20, 10, 5, 0)
...
def add_frame(fig: plt.Figure, section: str, number: int) -> None:
    fig.text(0.035, 0.969, "DMForPU · RTS 数据集图册", fontsize=9, weight="bold", color=MUTED)
    ...
    fig.text(0.035, 0.022, "正式数据：data/RTS{128,64,32} · 固定样本索引与统一色标", fontsize=7.4, color=MUTED)
...
def ids_by_region(split: str = "test_10dB") -> list[int]:
    with h5py.File(ROOT / "data" / "RTS128" / f"{split}.h5", "r") as f:
        regions = f["region_id"][:]
...
def cover_page(stats: dict, selected: dict, paths: list[Path]) -> None:
    ...
    fig.text(0.065, 0.855, "RTS", fontsize=39, weight="bold", color="white")
    fig.text(0.065, 0.765, "真实地形驱动相位解缠数据集图册", fontsize=25, weight="bold", color="white")
    fig.text(0.067, 0.700, "Real-Terrain Simulation Dataset Atlas", fontsize=12.5, color="#c9d8ea")
    ...
    rows = [
        ...
        ("噪声档位", "训练：0/5/10/20/30/60 dB；测试：clean + 0/5/10/20/30 dB"),
    ]
    y = 0.505
...
def dem_page(regions: list[dict], paths: list[Path]) -> None:
    ...
    for i, (region, ax) in enumerate(zip(regions, axes.flat)):
        tif = ROOT / "data" / "rts_sources" / "copernicus_glo30" / f"{region['name']}.tif"
...
def clean_page(ids: list[int], paths: list[Path]) -> None:
    ...
    fig.suptitle("RTS128 干净展开相位形态",x=.04,y=.905,ha="left",fontsize=19,weight="bold")
...
def deformation_page(paths: list[Path]) -> list[int]:
    path=ROOT/"data"/"RTS128"/"train.h5"
    with h5py.File(path,"r") as f:
...
def statistics_page(stats: dict, paths: list[Path]) -> None:
    image=plt.imread(ROOT/"data"/"RTS_statistics.png")
    ...
    items=["21 / 21 文件通过","33,000 条样本记录","所有相位数组均为有限值","包裹关系误差 = 0","跨 SNR 元数据一致","跨尺度来源一致"]
...
def assemble(paths: list[Path]) -> None:
    ...
    doc.setTitle("RTS Dataset Atlas"); doc.setAuthor("DMForPU")
    for path in paths:
...
def main() -> None:
    ...
    config=yaml.safe_load((ROOT/"configs"/"rts"/"regions.yaml").read_text(encoding="utf-8"))
    stats=json.loads((ROOT/"data"/"RTS_STATISTICS.json").read_text(encoding="utf-8"))["train_distribution_summary"]
    vector=shapefile.Reader(str(REGION_DIR/"rts_spatial_splits.shp"),encoding="utf-8")
