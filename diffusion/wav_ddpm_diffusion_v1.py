...
class WavDDPMDiffusion:
    ...
    def train_sample(self, t):
        ...
        mean = self.wrapped_cond.mean(dim=(0,2,3), keepdim=True)  # [1,C,1,1]
        std = self.wrapped_cond.std(dim=(0,2,3), keepdim=True) + 1e-8
        cond_normalized = (self.wrapped_cond - mean) / std
        # cond_summed = cond_normalized.sum(dim=1, keepdim=True)
        cond_avg = cond_normalized.mean(dim=1, keepdim=True)
        model_input = (self.noisy + cond_avg)/2
