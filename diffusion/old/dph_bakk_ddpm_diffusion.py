...
from utils.util import ExtraTokenCondition, UNetFeatureHook, AttentionKVInjector
...
class DphDDPMDiffusion:
    def __init__(self, config):
        ...
        self.kv_injector = AttentionKVInjector(self.unet)
    ...
    def feature_to_tokens(self,feat):
        # feat: [B, C, H, W]
        B, C, H, W = feat.shape
    ...
    def train_sample(self, t):
        ...
        selected_feats = {
            "down1": self.feature_to_tokens(hook.features["down1"]),  # [B, 64, H/8, W/8]
            "down2": self.feature_to_tokens(hook.features["down2"]),  # [B, 128, H/8, W/8]
            "down3": self.feature_to_tokens(hook.features["down3"]),  # [B, 256, H/8, W/8]
            "down4": self.feature_to_tokens(hook.features["down4"]),  # [B, 512, H/8, W/8]
            "mid": self.feature_to_tokens(hook.features["mid"]),    # [B,1024, H/8, W/8]
        }
