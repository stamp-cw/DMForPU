...
@register_diffusion(name='MchNegNormDDPMDiffusion')
class MchNegNormDDPMDiffusion:
    def __init__(self, config):
        self.config = config
        self.device = config.training.device
        self.model = UNet2DConditionModel(
            sample_size=config.model.sample_size,
            in_channels=config.model.in_channels + 6 + (config.diffusion.conditioning_channels if config.diffusion.on_conditioning and self.config.diffusion.fusion_type == 'concat' else 0),
        ...
        if self.config.diffusion.on_conditioning:
            if self.config.diffusion.use_controlnet:
                ...
                if self.config.diffusion.fusion_type == 'concat':
                    # model_input = torch.cat([self.noisy, self.wrapped_cond], dim=1)
                    model_input = torch.cat([self.noisy,self.noisy,self.noisy,self.noisy,self.noisy,self.noisy,self.noisy, self.wrapped_cond], dim=1)
                else:
            ...
            if self.config.diffusion.use_controlnet:
                ...
                for t in tqdm.tqdm(scheduler.timesteps, desc="Sampling"):
                    if self.config.diffusion.fusion_type == 'concat':
                        ...
                        model_input = torch.cat([x,x,x,x,x,x,x, self.wrapped_cond], dim=1)
                    else:
