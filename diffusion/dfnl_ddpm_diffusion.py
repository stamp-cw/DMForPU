...
class DFNDDPMDiffusion:
    ...
    def setup_data(self, batch_dict):
        ...
        self.gt_unwrapped_sub_wrapped_neg_norm = batch_dict["unwrapped_sub_wrapped_neg_norm"].to(self.device)
    ...
    def train_sample(self, t):
        ...
        encoder_hidden_states = None if cross_dim is None else torch.zeros(self.wrapped.shape[0], 1, cross_dim,
                                                                           device=self.device)
        self.noise = torch.randn_like(self.gt_unwrapped_sub_wrapped_neg_norm).to(self.device)
        self.noisy = self.scheduler.add_noise(self.gt_unwrapped_sub_wrapped_neg_norm, self.noise, t).to(self.device)
        ...
        self.pred_unwrapped_sub_wrapped_neg_norm = self.scheduler.step(self.noise_pred, t[0].cpu(), self.noisy).pred_original_sample
        self.pred_unwrapped_sub_wrapped_norm = (self.pred_unwrapped_sub_wrapped_neg_norm + 1) / 2
        self.pred_unwrapped = self.wrapped + self.pred_unwrapped_sub_wrapped_norm * (2 * torch.pi * (self.config.data.k_max - self.config.data.k_min))
    ...
    def infer_sample(self):
        ...
        if self.config.diffusion.on_conditioning:
            ...
            for t in tqdm.tqdm(scheduler.timesteps, desc="Sampling"):
                ...
                x = scheduler.step(self.noise_pred, t, x).prev_sample
        self.pred_unwrapped_sub_wrapped_neg_norm = x
        self.pred_unwrapped_sub_wrapped_norm = (self.pred_unwrapped_sub_wrapped_neg_norm + 1) / 2
        self.pred_unwrapped = self.wrapped + self.pred_unwrapped_sub_wrapped_norm * (2 * torch.pi * (self.config.data.k_max - self.config.data.k_min))
        self.diff_unwrapped = self.gt_unwrapped - self.pred_unwrapped
