...
class UpB(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.up = nn.ConvTranspose2d(
            in_ch, out_ch, kernel_size=3, stride=2, padding = 1, output_padding = 1 )
        self.res = ResB(out_ch*2, out_ch)
    def forward(self, x_q, x_k, x_v, x_):
        x0 = self.c_attn(x_q, x_k, x_v)
        x1 = self.up(x0)
        x2 = cat((x1 , x_), dim=1)
        x3 = self.res(x2)
...
class UNet(nn.Module):
    ...
    def forward(self, x, feats=False, down_feats=None, up_feats=None):
        if feats:
            ...
            x9  = self.up4(x8, u_feat_3, u_feat_3, x1_)
        else:
            x1, x1_ = self.down1(x, x, x)
            x2, x2_ = self.down2(x1, x1, x1)
            x3, x3_ = self.down3(x2, x2, x2)
            x4, x4_ = self.down4(x3, x3, x3)
            x5  = self.res(x4)
            x6  = self.up1(x5, x5, x5, x4_)
            x7  = self.up2(x6, x6, x6, x3_)
