...
class FDBasicTransformerBlock(BasicTransformerBlock):
    def __init__(
        ...
        self.xk_conv = nn.Conv2d(dim*3, cross_attention_dim, kernel_size=1)
    ...
    def forward(
        ...
        if self.attn2 is not None:
            ...
            _, _, emb_dim = encoder_hidden_states.shape
            ...
            x_k = x_k.view(B, 4, D*3, H//2, W//2) # (B,4,64*3,8,8)
            x_k = x_k.view(B*4, D*3, H//2, W//2) # (B*4, 64*3, 8, 8)
            x_k = self.xk_conv(x_k) # (B*4, emb_dim, 8, 8)
            x_k = x_k.view(B, 4, emb_dim, H//2, W//2) # (B, 4, emb_dim, 8, 8)
            x_k = x_k.permute(0, 1, 3, 4, 2).contiguous() # (B,4,8,8,64*3)
            x_k = x_k.view(B, N, emb_dim) # (B,256,64*3)
